...
const Kasir = ({ user }) => {
  ...
  const loadReceiptLogo = useCallback(async () => {
    if (receiptLogoRef.current) return receiptLogoRef.current;
    try {
      const response = await fetch("/stiker%20logo%20snapme.png");
      ...
      receiptLogoRef.current = base64;
      return base64;
    } catch (error) {
      console.warn("Logo nota gagal dimuat:", error);
      return null;
    }
  }, []);
  ...
  const changeAmount = useMemo(
    () => (parseFloat(customerPayment) || 0) - totalAmount,
    [customerPayment, totalAmount]
  );

  useEffect(() => {
    if (paymentMethod !== "Cash") setCustomerPayment("");
  }, [paymentMethod]);
  ...
  const handleSetFotogroupPrice = useCallback(
    (itemId, price) => {
  ...
  const generateReceipt = async () => {
    ...
    if (paymentMethod === "Cash") {
      ...
      if (payment < totalAmount)
        return alert(
          `Pembayaran kurang. Total: Rp ${formatRupiah(totalAmount)}`
        );
    }

    setLoading(true);
    try {
      const logoDataUrl = await loadReceiptLogo();
      const dynamicHeight = Math.max(120, 80 + cartItems.length * 8);
      const doc = new jsPDF({
        unit: "mm",
        format: [58, dynamicHeight],
      });
      const margin = 4;
      const contentWidth = 58 - margin * 2;
      let cursorY = margin;

      const addDivider = () => {
        doc.setDrawColor(230, 230, 230);
        doc.line(margin, cursorY, 58 - margin, cursorY);
        cursorY += 2.5;
      };
      ...
      const addLabel = (label, value) => {
        ...
        doc.text(value, margin + contentWidth, cursorY, { align: "right" });
        cursorY += 4;
      };
      ...
      if (logoDataUrl) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(12);
        doc.text("Snap Me!", 29, cursorY, { align: "center" });
        cursorY += 6;
      }

      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.text("Snap Me Self & Photo Studio", 29, cursorY, { align: "center" });
      cursorY += 4;
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7);
      doc.text("Jl. Nusa Indah No. 8, Singaraja", 29, cursorY, {
        align: "center",
      });
      cursorY += 3.5;
      ...
      addLabel("Customer", customerName);
      addLabel("Kasir", user.full_name);
      addLabel("Pembayaran", paymentMethod === "Cash" ? "Cash" : "Transfer");
      addDivider();

      doc.setFont("courier", "bold");
      doc.setFontSize(8);
      doc.text("Item", margin, cursorY);
      doc.text("Qty", margin + contentWidth - 24, cursorY);
      doc.text("Subtotal", margin + contentWidth, cursorY, { align: "right" });
      cursorY += 4;
      ...
      cartItems.forEach((item) => {
        ...
        doc.text(
          `${item.qty} x ${formatRupiah(item.effectivePrice)}`,
          margin,
          cursorY + 3,
          { baseline: "top" }
        );
        doc.text(String(item.qty), margin + contentWidth - 20, cursorY, {
          align: "right",
        });
        doc.text(
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8);
      doc.text(`Dibayar`, margin, cursorY);
      doc.text(`Rp ${formatRupiah(payment)}`, margin + contentWidth, cursorY, {
        align: "right",
      });
      cursorY += 4;
      doc.text(change >= 0 ? `Kembalian` : `Kekurangan`, margin, cursorY);
      doc.text(
        `Rp ${formatRupiah(Math.abs(change))}`,
        margin + contentWidth,
        cursorY,
        { align: "right" }
      );
      ...
      doc.setFontSize(7);
      doc.text("Terima kasih telah berkunjung!", 29, cursorY, {
        align: "center",
      });
      cursorY += 3;
      ...
      doc.save(`snapme-receipt-${transactionNumber}.pdf`);
              ...
              {itemsLoading ? (
                ...
                <div className="space-y-6">
                  <ItemSection
                    ...
                    items={items.studio}
                    cart={cart}
                    fotogroupPrices={fotogroupPrices}
                    onAdd={handleAddItem}
          ...
          <CardContent className="flex-1 flex flex-col gap-4 overflow-hidden">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {item.qty} x Rp {formatRupiah(item.effectivePrice)}
                        </div>
                      </div>
                ...
                <>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCustomerPayment(e.target.value.replace(/\D/g, ""))
                      }
                      ...
                      {[10000, 20000, 50000, 100000].map((amt) => (
                        <Button
                          ...
                          onClick={() =>
                            setCustomerPayment(
                              (
                                (parseInt(customerPayment) || 0) + amt
                              ).toString()
                            )
                          }
                        >
                    ...
                    <div className="grid grid-cols-2 gap-1 mt-1">
                      <Button
                        ...
                        className="text-xs h-7"
                        onClick={() =>
                          setCustomerPayment(totalAmount.toString())
                        }
                      >
                        Uang Pas
                      </Button>
            ...
            >
              {loading ? (
                ...
                <>
                  <Receipt className="w-4 h-4 mr-2" /> Buat Nota
                </>
              )}
            </Button>
          </CardContent>
...
const ItemSection = React.memo(
  ({
    ...
    <div>
      <h3 className="text-sm font-semibold text-muted-foreground mb-3 px-2 py-1 bg-muted rounded flex items-center gap-2">
        {icon} {title}
      </h3>
      ...
      <Card className="w-96">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <Button
              className="flex-1"
              onClick={() => price && onSetPrice(price)}
            >
              Set Harga
            </Button>
          </div>
